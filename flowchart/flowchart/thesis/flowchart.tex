...
\pgfooclass{module}{
 ...
 \method place2(#1) {
	...
	\node[below right= 0.1cm, text width= 2cm] at (\pgfoovalueof{label}.north west) {\pgfoovalueof{text}};
 }
...
\mod.set and draw(surfobs 1, SURFACE OBSERVATIONS, 0, 0)
\mod.set and place(first guess method, FIRST GUESS METHOD, below= 2cm of surfobs 1, left=2cm of surfobs 1)
\mod.set and place(forward model, FORWARD MODEL, below= 2cm of first guess method)
\mod.set and place(cost function, COST FUNCTION, below= 2cm of forward model)
\mod.set and place(surfobs 2, SURFACE OBSERVATIONS, above right= 0.1cm and 2.5cm of cost function)
\mod.set and place(bedobs 2, BED OBSERVATIONS, below right= 0.1cm and 2.5cm of cost function)


\mod.set width(6cm)
\mod.set text width(5.5cm)
\mod.set height(6.5cm)

\mod.set and place2(stepping algorithm, STEPPING ALGORITHM,  below= 1.8cm of cost function)

\mod.set width(3.5cm)
\mod.set text width(3cm)
\mod.set height(1.5cm)

\mod.set and place diamond(cost minimal, COST MINIMAL?,  below= 2.5cm of cost function)
\mod.set and place(update bed, UDATE BED ESTIMATE, below= 2cm of cost minimal)
\mod.set width(3.5cm)
\mod.set text width(3cm)
\mod.set height(3cm)
\mod.set and place(optimal bed, FOUND OPTIMAL BED, right=2.5 cm of update bed)

%\mod.connect(surfobs 1, first guess method)

\draw[-stealth,pencil draw,thick,decorate,dashed] (surfobs 2.north)|-( first guess method.east);
...
\mod.connect around2(cost minimal.east, optimal bed.north)
...
\node[font=\cmcsans, fill=white] at ($(first guess method)!0.5!(forward model)$) {BED ESTIMATE};
\node[fill=white] at ($(forward model)!0.5!(cost function)$) {MODELLED SURFACE};
\node[fill=white] at ($(cost function)!0.5!(cost minimal)+(0,0.2)$) {\ \ \ \ COST + GRADIENT};
\node at ($(cost minimal)!0.5!(update bed)+(0.50,0.63)$) {NO};
\node at ($(cost minimal)+(2.5,0.28)$) {YES};
