...
def histalp_commitment_custom_scaling(path=True):
    ...
    scaling_params_dict = json.load(open('/Users/oberrauch/work/master/data/hef_sensitivity/scaling_params.json'))
    ...
    sensitivity_run_vas(rgi_ids=rgi_ids, scaling_params=scaling_params_list,
                        suffixes=suffixes, temp_bias=+0.5, path=path,
                        use_bias_for_run=False, tstar=1927)


def histalp_timescale_sensitivity(path=True):
    """Use new found scaling parameters for HISTALP commitment run"""
    # start logger with OGGM settings
    cfg.set_logging_config()

    # define file suffixes
    suffixes = ['_half', '_default', '_twice']

    # get HISTALP RGI IDs
    # rgi_ids = pd.read_csv('/home/users/moberrauch/data/histalp_rgi_ids.csv', index_col=0)['RGIId'].values
    rgi_ids = ['RGI60-11.00897']

    sensitivity_run_vas(rgi_ids=rgi_ids, time_scale_factors=[0.5, 1, 2],
                        suffixes=suffixes, temp_bias=+0.5, path=path,
                        use_bias_for_run=False, tstar=1927)
