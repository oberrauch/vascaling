""" Hereafter I set up a full run with the OGGM flowline model from
start to finisch. This includes:
a) initialisation and calibration
b) the mass balance model
c) the 'dynamic' model

The results are stored under `run_oggm.csv` in the `../data/` directory.
The plots show the temporal evolution of glacier length, surface area
and volume and are store under `length/area/volume_oggm.png` in the
`../plots/` directory.
...
# create plots and store to file
plot = False
if plot:
    # define path and file names
