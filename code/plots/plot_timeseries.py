...
def plot_both_climates(ds, var, title=['Volume/area scaling model', 'Flowline model'], xlim=None):
    ...
    # flowline model
    ds.sel(model='fl', mb_model='random')[var].plot(hue='temp_bias', ax=ax_vas, add_legend=False, color='lightgray',
                                                      lw=0.5)
    # vas model
    ax_vas.set_prop_cycle('color', vas_cycle)
    handles_vas_r = ds.sel(model='vas', mb_model='random')[var].plot(hue='temp_bias', ax=ax_vas, add_legend=False, lw=2)
    handles_vas_c = ds.sel(model='vas', mb_model='constant')[var].plot(hue='temp_bias', ax=ax_vas, add_legend=False,
                                                                       lw=1.7, ls='--')
    # define legend labels
    labels_vas = ["$\\bf{const:}$", "$\\bf{rand:}$"]
    labels_vas.extend(np.array(['{:+.1f} °C'.format(bias) for bias in ds.temp_bias.values]).repeat(2))
    # define legend handles
    title_proxy, = plt.plot(ds.sel(model='vas', mb_model='random')[var].isel(temp_bias=0, time=0).values,
    ...
    ax_vas.legend(handles_vas, labels_vas, title='', bbox_to_anchor=(0.5, 1), loc=8, ncol=4)
    if ds.normalized:
        # add ylabel
        ...
        # aux line
        ax_vas.axhline(ds.sel(model='vas')[var].isel(time=0).mean(), lw=0.8, ls=':', c='k')
    ...
    # vas model
    ds.sel(model='vas', mb_model='random')[var].plot(hue='temp_bias', ax=ax_fl, add_legend=False, color='lightgray',
                                                       lw=0.5)
    # flowline model
    ax_fl.set_prop_cycle('color', fl_cycle)
    handles_fl_r = ds.sel(model='fl', mb_model='random')[var].plot(hue='temp_bias', ax=ax_fl, add_legend=False, lw=2)
    handles_fl_c = ds.sel(model='fl', mb_model='constant')[var].plot(hue='temp_bias', ax=ax_fl, add_legend=False,
                                                                     lw=1.7, ls='--')
    ...
    ax_fl.legend(handles_fl, labels_fl, bbox_to_anchor=(0.5, 1.0), loc=8, ncol=4)
    ...
    if ds.normalized:
        ...
        # aux line
        ax_fl.axhline(ds.sel(model='fl')[var].isel(time=0).mean(), lw=0.8, ls=':', c='k')
...
def plot_both_climates_same_fig(ds, var, title='', suptitle='', xlim=None):
    ...
    # flowline model
    ds.sel(model='fl', mb_model='constant')[var].plot(hue='temp_bias', ax=ax0, add_legend=False, color='lightgray',
                                                      lw=0.5)
    # vas model
    ax0.set_prop_cycle('color', vas_cycle)
    handles_vas_r = ds.sel(model='vas', mb_model='random')[var].plot(hue='temp_bias', ax=ax0, add_legend=False, lw=2)
    handles_vas_c = ds.sel(model='vas', mb_model='constant')[var].plot(hue='temp_bias', ax=ax0, add_legend=False,
                                                                       lw=1.7, ls='--')
    ...
    # vas model
    ds.sel(model='vas', mb_model='constant')[var].plot(hue='temp_bias', ax=ax1, add_legend=False, color='lightgray',
                                                       lw=0.5)
    # flowline model
    ax1.set_prop_cycle('color', fl_cycle)
    handles_fl_r = ds.sel(model='fl', mb_model='random')[var].plot(hue='temp_bias', ax=ax1, add_legend=False, lw=2)
    handles_fl_c = ds.sel(model='fl', mb_model='constant')[var].plot(hue='temp_bias', ax=ax1, add_legend=False, lw=1.7,
                                                                     ls='--')
    ...
    for rgi_id, glacier in showcase_glaciers.iterrows():
        ...
        for var in variables:
            ...
            for norm in [True]:
                ...
                figs = plot_both_climates(ds.sel(normalized=norm, rgi_id=rgi_id),
                ...
                for m, fig in zip(['vas', 'fl'], figs):
                    ...
                    f_path += '{}_{}_{}_{}.pdf'.format(var, 'norm' if norm else 'abs', m,
                                                name.replace(' ', '_'))
    ...
    for mb_model in ds['mb_model'].values:
        ...
        for var in variables:
            ...
            for norm in [True, False]:
                ...
                suptitle = None
                plot_time_series(ds.sel(mb_model=mb_model, normalized=norm, rgi_id='sum'),
                                 var=var, suptitle=suptitle, xlim=xlim)
                # store to file
                f_path = '/Users/oberrauch/work/master/plots/final_plots/time_series/histalp_commitment/'
                f_path += '{}_{}_{}.pdf'.format(var, 'norm' if norm else 'abs', mb_model)
                plt.savefig(f_path, bbox_inches='tight')
                plt.close(plt.gcf())
...
def plot_histalp_commitment_both_climates():
    ...
    for var in variables:
        ...
        for norm in [True, False]:
            ...
            for m, fig in zip(['vas', 'fl'], figs):
                f_path = '/Users/oberrauch/work/master/plots/final_plots/time_series/histalp_commitment/'
                f_path += '{}_{}_{}.pdf'.format(var, 'norm' if norm else 'abs', m)
                fig.savefig(f_path, bbox_inches='tight')
                plt.close(fig)
